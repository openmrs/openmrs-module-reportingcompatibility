...
public class RowPerObsDataSetProvider implements DataSetProvider {
	...
	public DataSet<?> evaluate(DataSetDefinition dataSetDefinition, Cohort inputCohort, EvaluationContext evalContext) {
		...
		ReportingCompatibilityService rcs = (ReportingCompatibilityService) Context.getService(ReportingCompatibilityService.class);
		if (definition.getFilter() != null) {
			if (patients != null)
				patients = Cohort.intersect(patients, rcs.evaluate(definition.getFilter(),
				    evalContext));
			else
				patients = rcs.evaluate(definition.getFilter(), evalContext);
		}
