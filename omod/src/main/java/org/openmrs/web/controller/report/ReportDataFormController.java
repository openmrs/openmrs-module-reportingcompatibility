...
public class ReportDataFormController extends SimpleFormController {
	...
	protected Map<String, Object> referenceData(HttpServletRequest request) throws Exception {
		...
		ReportData report = (ReportData) request.getSession().getAttribute(ReportingCompatibilityConstants.OPENMRS_REPORT_DATA);
		...
		if (Context.isAuthenticated() && report != null && "rerender".equals(request.getParameter("action"))) {
			...
			if (renderer instanceof WebReportRenderer) {
				...
				if (webRenderer.getLinkUrl(schema) != null) {
					request.getSession().setAttribute(ReportingCompatibilityConstants.OPENMRS_REPORT_DATA, report);
					request.getSession().setAttribute(ReportingCompatibilityConstants.OPENMRS_REPORT_ARGUMENT, renderArg);
