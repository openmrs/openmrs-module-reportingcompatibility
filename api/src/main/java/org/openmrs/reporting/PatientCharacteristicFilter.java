...
public class PatientCharacteristicFilter extends CachingPatientFilter implements Comparable<PatientCharacteristicFilter> {
	...
	public String getDescription() {
		...
		if (minDeathdate != null) {
			if (maxDeathdate != null) {
				ret.append(" ").append(
						msa.getMessage("reportingcompatibility.reporting.diedBetween", new Object[] { (Object) df.format(minDeathdate),
								(Object) df.format(maxDeathdate) }, Context.getLocale()));
			} else {
				ret.append(" ").append(msa.getMessage("reportingcompatibility.reporting.diedAfter")).append(" ").append(df.format(minDeathdate));
			}
		} else {
			if (maxDeathdate != null) {
				ret.append(" ").append(msa.getMessage("reportingcompatibility.reporting.diedBefore")).append(" ").append(df.format(maxDeathdate));
			}
		}
	...
	public Cohort filterImpl(EvaluationContext context) {
		return Context.getService(ReportingCompatibilityService.class).getPatientsByCharacteristics(gender, minBirthdate, maxBirthdate, minDeathdate, maxDeathdate, minAge, maxAge, aliveOnly, deadOnly, effectiveDate);
	}
}